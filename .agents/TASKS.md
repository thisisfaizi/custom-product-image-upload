...
| T-006 | Pro's bulk save silently forces `disable_quantity` to `false` on every product it touches — the field has no control in the Bulk Operations form at all, so it's always absent from the POST and the base sanitizer defaults it | `@sec` | `in_review` | browser verify | **Correction (2026-07-26):** the original framing (that `enable_shape_cropping`/`resolution_validation`/dimensions are *also* silently reset) was wrong — `cpiu-admin-pro.js:87-93` force-sets all of those into `FormData` on every submit regardless of checkbox state, so they're deliberately overwritten, not silently reset. Only `disable_quantity` is genuinely missing. Decision recorded in `CONTEXT.md`: option (a), widen Pro's bulk form (fixed in the **Pro repo**, `class-cpiu-pro-bulk.php` + `cpiu-admin-pro.js`, tracked there as P-034) — not (b), touching the shared base sanitizer. No base-repo code changed. Pure HTML/JS fix with no headless-PHP verification path — needs the same browser access as T-007 |
